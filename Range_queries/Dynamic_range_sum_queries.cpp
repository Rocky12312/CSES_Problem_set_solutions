...
ll query_tree_sum(query_node temp,ll start,ll end,ll idx,ll sg_tree[]){
	...
	ll mid = (start+end)/2;
	ll left = query_tree_sum(temp,start,mid,2*idx,sg_tree);
	ll right = query_tree_sum(temp,mid+1,end,2*idx+1,sg_tree);
	return left+right;
}
...
void solve(ll n,ll q,ll arr[],query_node q_arr[]){
	...
	for(ll i=0;i<q;i++){
		...
		if(temp.a == 1){
			...
			temp.b = temp.b-1;
			temp.c = temp.c-1;
			ll res = query_tree_sum(temp,0,n-1,1,sg_tree);
			cout<<res<<endl;
		}
	}
	return;
